...
def prepare_features(cluster_df: pd.DataFrame, validation_df: pd.DataFrame) -> tuple:
    ...
    cluster_counts = cluster_df.groupby(['sample', 'cluster']).size().reset_index(name='count')
    
    # Pivot to create feature matrix (samples x clusters)
    feature_matrix = cluster_counts.pivot(index='sample', columns='cluster', values='count')
    ...
    logger.info(f"Common samples between clusters and validation: {len(common_samples)}")
    ...
    additional_features = ['metadata_completeness', 'fungal_signal', 'read_pairs', 'host_contamination']
...
def main():
    parser = argparse.ArgumentParser(description="Train classification model from protein clusters")
    parser.add_argument("--clusters", required=True, help="Input CSV file with cluster assignments")
    parser.add_argument("--validation-data", nargs='+', required=True, help="Validation CSV files")
    parser.add_argument("--output", required=True, help="Output pickle file for trained model")
    parser.add_argument("--test-size", type=float, default=0.2, help="Test set size fraction")
    parser.add_argument("--random-state", type=int, default=42, help="Random state for reproducibility")
    parser.add_argument("--threads", type=int, default=-1, help="Number of threads for model training")
