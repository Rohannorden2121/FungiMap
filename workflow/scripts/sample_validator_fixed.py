...
class SampleValidator:
    ...
    def __init__(self, config_path: Path, criteria: Optional[ValidationCriteria] = None, batch_size: int = 10, max_workers: int = 4):
    ...
    async def _check_metadata_completeness(self, accession: str) -> Tuple[float, int]:
        ...
        try:
            ...
            raw_completeness = (valid_values / total_fields) * 100 if total_fields else 0.0
    ...
    async def validate_sample(self, accession: str) -> ValidationResult:
        ...
        try:
            ...
            metadata_score, valid_values = await self._check_metadata_completeness(accession)
            ...
            if not passes_all:
                required_fields = len(self.config["validation"]["required_metadata_fields"])
    ...
    async def _validate_sequence_data(self, accession: str) -> Dict[str, float]:
        ...
        try:
            ...
            fastqc_path = self.storage_root / "eda" / "fastqc" / f"{accession}_fastqc.zip"
                ...
                with zf.open(f"{accession}_fastqc/fastqc_data.txt") as f:
                    ...
                    for line in data:
                        if line.startswith('Total Sequences'):
                            metrics['read_pairs'] = int(line.split()[-1]) // 2  # Paired-end reads
                    ...
                    for line in data:
                        if line.startswith('#Per base sequence quality'):
                            ...
                            try:
                                ...
                                parts = line.split()
                                if len(parts) >= 2:  # Ensure we have at least base and mean
                                    quality_scores.append(float(parts[1]))  # Mean quality
                            except (ValueError, IndexError):
                                continue
                    
                    if quality_scores:
                        metrics['mean_quality'] = sum(quality_scores) / len(quality_scores)
            
            self.logger.debug(f"Sequence metrics for {accession}: {metrics}")
            
        except Exception as e:
            self.logger.error(f"Error processing FastQC results for {accession}: {str(e)}")
    ...
    async def _analyze_taxonomic_composition(self, accession: str) -> Dict[str, float]:
        ...
        try:
            ...
            kraken_report = self.storage_root / "eda" / "kraken2" / f"{accession}_report.txt"
            bracken_report = self.storage_root / "eda" / "bracken" / f"{accession}_bracken.txt"
            ...
            if bracken_report.exists():
                ...
                sorted_species = sorted(species_abundances.items(), 
                                      key=lambda x: x[1], 
                                      reverse=True)
            ...
            self.logger.error(f"Error processing taxonomic results for {accession}: {str(e)}")
        
        return metrics
    
    async def validate_sample_batch(self, accessions: List[str]) -> Dict[str, ValidationResult]:
    ...
    async def validate_sample_batch(self, accessions: List[str]) -> Dict[str, ValidationResult]:
        ...
        for i in range(0, total_samples, self.batch_size):
            ...
            self.logger.info(f"Processing batch {batch_num}/{total_batches} ({len(batch)} samples)")
            
            # Use ThreadPoolExecutor for I/O bound validation tasks
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                # Create tasks for this batch
                tasks = [
                    asyncio.get_event_loop().run_in_executor(executor, self._validate_single_sample, accession)
            ...
            self.logger.info(f"Batch {batch_num}/{total_batches} completed - {progress:.1f}% total progress")
        
        self.logger.info(f"Batch validation completed: {self.processed_count} successful, {self.failed_count} failed")
        return results
    ...
    def generate_validation_report(self, results: Dict[str, ValidationResult]) -> pd.DataFrame:
        ...
        for acc, result in results.items():
            row = {
                ...
                "metadata_completeness": result.metrics.get("metadata_completeness", 0.0),
        ...
        self.logger.info(f"  Success rate: {(len(df[df['Status'] == 'PASS']) / len(df) * 100):.1f}%")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 4:
        print("Usage: sample_validator.py <fastq_file> <validation_config> <output_report>")
        sys.exit(1)
