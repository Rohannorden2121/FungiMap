...
"""
    ...
    def estimate_resources(self, task_type: str, input_size_gb: float) -> Dict:
        ...
        multipliers = RESOURCE_MULTIPLIERS.get(task_type, {
            "memory_per_gb": 1,
            "cpu_per_gb": 1,
            "time_per_gb": 1
        })
    ...
    def prepare_job(self, 
                   ...
                   commands: list[str],
                   job_name: str,
                   provider: str = "aws") -> str:
        """Prepare a cloud job script"""
        resources = self.estimate_resources(task_type, input_size_gb)
        
        if provider == "aws":
            instance_type = self.get_instance_type(resources["memory_gb"], resources["cpus"])
