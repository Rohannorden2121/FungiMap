...
class ProductionMonitor:
    ...
    def get_slurm_jobs(self) -> List[Dict]:
        ...
        try:
            result = subprocess.run(['squeue', '-u', '$USER', '--format=%i,%j,%t,%M,%N'], 
        ...
        try:
            ...
            for stage_name, stage_path in stage_dirs.items():
                if stage_path.exists():
                    ...
                    analysis['total_size_gb'] += analysis['stages'][stage_name]['size_gb']
                else:
            ...
            if validation_reports:
                ...
                combined_report = results_path / "eda" / "validation" / "combined_report.csv"
                if combined_report.exists():
                    try:
                        ...
                        analysis['success_rate'] = round(analysis['passed_samples'] / len(df) * 100, 1)
                    except Exception:
                        pass
        ...
        try:
            while report_count < max_reports:
                ...
                if 'cpu_percent' in resources:
                    self.logger.info(f"System Status - CPU: {resources['cpu_percent']:.1f}%, "
                                   f"Memory: {resources['memory_percent']:.1f}%, "
                                   f"Disk: {resources['disk_percent']:.1f}%")
                
                slurm_jobs = report.get('slurm_jobs', [])
                if slurm_jobs:
                    running_jobs = [j for j in slurm_jobs if j['status'] in ['R', 'RUNNING']]
                    pending_jobs = [j for j in slurm_jobs if j['status'] in ['PD', 'PENDING']]
                    self.logger.info(f"SLURM Jobs - Running: {len(running_jobs)}, Pending: {len(pending_jobs)}")
                
                results = report.get('results_analysis', {})
                if 'validated_samples' in results:
                    self.logger.info(f"Pipeline Progress - Validated: {results['validated_samples']}, "
...
def main():
    ...
    parser.add_argument("--output-dir", default="results/monitoring",
    ...
    if args.mode == 'single':
        ...
        if 'cpu_percent' in resources:
            ...
            print(f"  Memory: {resources['memory_percent']:.1f}% ({resources['memory_used_gb']:.1f}GB used)")
            print(f"  Disk: {resources['disk_percent']:.1f}% ({resources['disk_used_gb']:.1f}GB used)")
