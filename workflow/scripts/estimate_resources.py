...
def main():
    ...
    for idx, row in pilot_samples.iterrows():
        resources = estimate_sample_resources(
            row["accession"],
            metadata[metadata["accession"] == row["accession"]].iloc[0],
        )
        sample_resources.append(resources)
        total_disk_needed += (
            resources["raw_size_gb"]
            + resources["assembly_temp_gb"]
            + resources["final_size_gb"]
        )
    ...
    report.append(
        f"- Estimated total runtime: {sum(r['estimated_runtime_hrs'] for r in sample_resources):.1f} hours"
    )
    ...
    if max_memory_needed > system_memory:
        report.append(
            "⚠️ WARNING: Some samples may need memory-efficient fallback (MEGAHIT)"
        )
    ...
    return (
        total_disk_needed <= disk_space["free_gb"]
